...
const PILL_STYLES: Record<PillKey, string> = {
  Doujinshi: "bg-[#fbe1ec] text-[#be3d78]",
  "Light Novel": "bg-[#dcf3e6] text-[#1f8a54]",
};

const STOCK_STYLES: Record<StockStatus, { dot: string; text: string }> = {
  "In Stock": { dot: "bg-[#16a34a]", text: "text-[#16a34a]" },
  Preorder: { dot: "bg-[#ea9a1c]", text: "text-[#ea9a1c]" },
};
...
const LN_SERIES: LNSeriesItem[] = [
  ...
  { img: "/luminnovel/cover-7thloop3.png", title: "86—EIGHTY-SIX", badge: "Light Novel", volumes: "9 volumes" },
];
...
function LNChrome() {
  return (
    <div className="flex items-center gap-2.5 border-b border-[#e7ddcd] bg-[#f3ede2] px-3.5 py-2.5">
      ...
      <div className="flex flex-1 justify-center">
        <div className="flex min-w-[150px] items-center justify-center gap-1.5 rounded-full border border-[#e7ddcd] bg-white px-3 py-1 text-[11px] text-[#78716c] sm:min-w-[200px] sm:px-3.5 sm:text-xs">
...
// Storefront header — LN monogram + nav + search pill. `compact` drops the
// nav row for screens that need more vertical room (Detail, Series).
function LNStoreHeader({ compact = false }: { compact?: boolean }) {
  return (
    <div className={`flex items-center gap-3.5 border-b border-[#e7ddcd] ${compact ? "px-4 py-2.5" : "px-4 py-3"}`}>
      <div className="flex items-center gap-2">
        <span className="flex h-[26px] w-[26px] items-center justify-center rounded-[7px] bg-[#1c1917] text-[11px] font-extrabold tracking-wide text-white">
          LN
        </span>
        <span className="text-sm font-bold text-[#1c1917]">Lumin Novel</span>
      </div>
      {!compact && (
        <div className="hidden gap-3.5 text-xs text-[#78716c] sm:flex">
          <span>Manga</span>
      ...
      <div
        className={`rounded-full border border-[#e7ddcd] bg-white px-3 py-1 text-[11px] text-[#a8a29e] ${
...
function LNHome() {
  return (
    <div className="flex h-full flex-col bg-[#faf7f1]">
      <LNStoreHeader />
      <div className="flex items-center justify-between px-4 pb-1.5 pt-3 sm:px-4">
        <span className="text-base font-bold text-[#1c1917] sm:text-[17px]">Catalog</span>
        <div className="hidden gap-2 sm:flex">
          <span className="rounded-md border border-[#e7ddcd] bg-white px-2 py-1 text-[10px] text-[#78716c]">All Formats ▾</span>
          <span className="rounded-md border border-[#e7ddcd] bg-white px-2 py-1 text-[10px] text-[#78716c]">Title A–Z ▾</span>
        </div>
      </div>
      <div className="grid flex-1 grid-cols-3 gap-2.5 overflow-hidden px-4 pb-3 pt-1 sm:grid-cols-5 sm:gap-3 sm:pb-4">
        {LN_PRODUCTS.map((p) => (
          <div key={p.title} className="group flex min-w-0 flex-col gap-1">
            ...
            <span className="line-clamp-2 text-[10px] leading-tight text-[#1c1917]">{p.title}</span>
            <span className="text-[11px] font-bold text-[#1c1917]">{p.price}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="flex h-full flex-col bg-[#faf7f1]">
      ...
      <div className="grid flex-1 grid-cols-1 items-center gap-5 overflow-hidden px-5 py-4 sm:grid-cols-[auto_1fr] sm:gap-6 sm:px-6">
        ...
        <div className="flex min-w-0 flex-col gap-2 sm:gap-2.5">
          <span className="text-[10px] text-[#78716c]">Light Novels › 7th Time Loop</span>
          <span className="text-base font-bold leading-tight text-[#1c1917] sm:text-xl">
          ...
          <span className="text-xl font-extrabold text-[#1c1917] sm:text-2xl">{p.price}</span>
          <div className="mt-1 flex items-center gap-2.5">
            <div className="flex items-center overflow-hidden rounded-lg border border-[#e7ddcd] bg-white">
              <span className="px-2.5 py-1.5 text-[#78716c]">–</span>
              <span className="border-x border-[#e7ddcd] px-3 py-1.5 text-[13px] font-semibold text-[#1c1917]">1</span>
              <span className="px-2.5 py-1.5 text-[#78716c]">+</span>
            </div>
            <span className="rounded-lg bg-[#1c1917] px-5 py-2 text-[13px] font-semibold text-white">Add to Cart</span>
          </div>
          <span className="mt-0.5 hidden text-[10px] text-[#78716c] sm:block">
...
// Screen 3 — series index: header + "N series available" + a card grid.
function LNSeries() {
  return (
    <div className="flex h-full flex-col bg-[#faf7f1]">
      <LNStoreHeader compact />
      <div className="flex items-center justify-between px-4 pb-1.5 pt-3 sm:px-4">
        <span className="text-base font-bold text-[#1c1917] sm:text-[17px]">Series</span>
        <span className="text-[10px] text-[#78716c] sm:text-xs">190 series available</span>
      </div>
      <div className="grid flex-1 grid-cols-3 gap-2.5 overflow-hidden px-4 pb-3 pt-1 sm:grid-cols-5 sm:gap-3 sm:pb-4">
        {LN_SERIES.map((s) => (
          <div key={s.title} className="group flex min-w-0 flex-col gap-1">
            <div className="relative aspect-[0.71] overflow-hidden rounded-md bg-white shadow-[0_4px_10px_rgba(28,25,23,0.14)] transition-shadow duration-300 group-hover:shadow-[0_10px_20px_rgba(28,25,23,0.22)]">
              <Image
                src={s.img}
                alt={s.title}
            ...
            <span className="line-clamp-2 text-[10px] leading-tight text-[#1c1917]">{s.title}</span>
            <span className="text-[9px] text-[#78716c]">{s.volumes}</span>
          </div>
        ))}
      </div>
    </div>
...
export default function LuminNovel() {
  ...
  return (
    <section id="freelance" aria-label="Freelance project: Lumin Novel" className="w-full border-y border-[#e7ddcd] bg-[#faf7f1]">
      <div className="container-max grid items-center gap-10 py-20 md:grid-cols-[5fr_6fr] md:gap-14 md:py-32">
        ...
        <div className="flex flex-col gap-5">
          ...
          <h2 className="text-5xl font-extrabold tracking-tight text-[#1c1917] md:text-6xl">
          ...
          <div className="flex flex-wrap gap-2">
            {TECH_CHIPS.map((c) => (
              <span key={c} className="rounded-full border border-[#e7ddcd] bg-white px-3 py-1.5 text-xs font-medium text-[#57534e]">
                {c}
              </span>
            ))}
          </div>
          <a
            ...
            className="mt-1 self-start text-base font-semibold text-[#1c1917] underline decoration-emerald-500 underline-offset-[5px]"
          >
        ...
        <div
          ...
          className="overflow-hidden rounded-2xl border border-[#e7ddcd] bg-white shadow-[0_24px_60px_rgba(28,25,23,0.22)]"
        >
          <LNChrome />
          ...
          {/* Screen switcher */}
          <div className="flex items-center justify-center gap-4 border-t border-[#e7ddcd] bg-[#f3ede2] px-2.5 py-2.5">
            {SCREENS.map((s, i) => (
              <button
                key={s.key}
                type="button"
                ...
                className={`flex items-center gap-1.5 text-[11px] transition ${
                  i === active ? "font-bold text-[#1c1917]" : "font-medium text-[#78716c]"
                }`}
              >
                <span
